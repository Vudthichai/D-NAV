...
import PdfDecisionIntake, {
  type AddDecisionResult,
  type AddMultipleResult,
} from "@/components/decision-intake/PdfDecisionIntake";
...
export default function StressTestPage() {
  ...
  const handleAddToSession = useCallback(
    (candidate: DecisionCandidate, sourceFile: string | null): AddDecisionResult => {
      const title = candidate.title.trim() || "Untitled decision";
      let nextTotal = sessionDecisions.length;
      let added = false;
      const decisionId = `extract-${candidate.id}`;
      setSessionDecisions((prev) => {
        if (prev.some((decision) => decision.id === decisionId)) return prev;
        const sessionDecision: SessionDecision = {
          id: decisionId,
          decisionTitle: title,
          ...
          excerpt: candidate.evidence.fullQuote || undefined,
          sourceType: "intake",
          createdAt: Date.now(),
        };
        added = true;
      ...
      return { added, decisionId, total: nextTotal };
    },
    [sessionDecisions.length],
  );

  const handleAddMultiple = useCallback(
    (candidates: DecisionCandidate[], sourceFile: string | null): AddMultipleResult => {
      if (candidates.length === 0) return { addedCount: 0, total: sessionDecisions.length };
      let addedCount = 0;
      let nextTotal = sessionDecisions.length;
      ...
      return { addedCount, total: nextTotal, lastAddedId: lastAdded };
    },
    [sessionDecisions.length],
  );

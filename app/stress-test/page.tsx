...
import { DEFINITION_LOOKUP } from "@/src/lib/definitions";
...
export default function StressTestPage() {
  ...
  const renderTooltipLabel = useCallback((label: string, key: keyof typeof DEFINITION_LOOKUP) => {
    const definition = DEFINITION_LOOKUP[key];
    return (
      <GlassyTooltip
        content={
          <div className="space-y-1">
            <div className="text-xs font-semibold text-foreground">{definition.title}</div>
            <div className="text-xs text-muted-foreground">{definition.body}</div>
          </div>
        }
      ...
      <div className="mx-auto max-w-6xl space-y-3 px-4 pb-4 pt-3 md:px-6">
              ...
              <CardContent className="flex flex-1 flex-col gap-2 px-4 pb-2">
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="space-y-0.5 rounded-md border border-border/60 bg-muted/20 px-2 py-0">
                      <SliderRow
                        id="impact"
                        label={renderTooltipLabel("Impact", "impact")}
                      ...
                      <SliderRow
                        id="cost"
                        label={renderTooltipLabel("Cost", "cost")}
                      ...
                      <SliderRow
                        id="risk"
                        label={renderTooltipLabel("Risk", "risk")}
                      ...
                      <SliderRow
                        id="urgency"
                        label={renderTooltipLabel("Urgency", "urgency")}
                      ...
                      <SliderRow
                        id="confidence"
                        label={renderTooltipLabel("Confidence", "confidence")}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-[11px] font-semibold text-muted-foreground">
                          {renderTooltipLabel("Return", "return")}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                  ...
                  <div className="rounded-md border border-border/60 bg-muted/30 px-2 py-1">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-[11px] font-semibold text-muted-foreground">
                          {renderTooltipLabel("Pressure", "pressure")}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                  ...
                  <div className="rounded-md border border-border/60 bg-muted/30 px-2 py-1">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-[11px] font-semibold text-muted-foreground">
                          {renderTooltipLabel("Stability", "stability")}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
        ...
        {process.env.NODE_ENV === "development" ? (
          <div className="rounded-lg border border-dashed border-border/60 bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
            Tooltip demo: {renderTooltipLabel("Impact", "impact")} · {renderTooltipLabel("Cost", "cost")} ·{" "}
            {renderTooltipLabel("Risk", "risk")} · {renderTooltipLabel("Urgency", "urgency")} ·{" "}
            {renderTooltipLabel("Confidence", "confidence")} · {renderTooltipLabel("Return", "return")} ·{" "}
            {renderTooltipLabel("Pressure", "pressure")} · {renderTooltipLabel("Stability", "stability")}
          </div>
        ) : null}
      </div>

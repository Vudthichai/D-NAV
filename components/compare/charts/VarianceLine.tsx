...
const metricPalette: Record<"R" | "P" | "S", string> = {
    ...
    <div className="space-y-2">
      {(title || subtitle) && (
        <div className="flex items-start justify-between gap-2">
          ...
          <p className="text-[11px] text-muted-foreground">Rolling σ for Return, Pressure, Stability.</p>
        </div>
      )}
            ...
            <LineChart margin={{ top: 12, right: 18, bottom: 12, left: 0 }}>
              ...
              {series.map((entry) => (
                <React.Fragment key={entry.id}>
                  <Line type="monotone" data={entry.data} dataKey="R" stroke={metricPalette.R} dot={false} name={`${entry.label} · Return σ`} strokeWidth={2} strokeDasharray={entry.id === "B" ? "4 2" : "none"} />
                  <Line type="monotone" data={entry.data} dataKey="P" stroke={metricPalette.P} dot={false} name={`${entry.label} · Pressure σ`} strokeWidth={2} strokeDasharray={entry.id === "B" ? "4 2" : "none"} />
                  <Line type="monotone" data={entry.data} dataKey="S" stroke={metricPalette.S} dot={false} name={`${entry.label} · Stability σ`} strokeWidth={2} strokeDasharray={entry.id === "B" ? "4 2" : "none"} />
                </React.Fragment>
              ))}
            </LineChart>
          </ResponsiveContainer>

...
type PillVariant = "green" | "amber" | "red" | "blue";

type LogMode = "enabled" | "link";

export type StressTestCalculatorHandle = {
  selectDecision: (decision: { text: string; category: string }) => void;
};
...
const StressTestCalculator = (
  { logMode = "enabled", logHref = "/stress-test", showFooter = true }: StressTestCalculatorProps,
  ref: Ref<StressTestCalculatorHandle>,
) => {
  ...
  const { setDecisions, isDatasetLoading, loadError } = useDataset();
  ...
  const getPillColor = useCallback((value: number, type: "return" | "stability" | "pressure") => {
    if (type === "pressure") {
            ...
            <GlassCard className="flex h-full flex-col gap-6 py-6">
              ...
              <CardContent className="flex flex-1 flex-col gap-2 px-4 pb-2">
                ...
                <div className="mt-auto flex flex-col gap-2 pt-1 sm:flex-row sm:items-center sm:justify-between">
                  <Button
                    onClick={isLogLink ? handleLogLink : handleSaveDecision}
                    variant="secondary"
...
export default forwardRef(StressTestCalculator);
